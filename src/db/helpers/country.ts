...
export const createUpdateCountriesAction = async (payload: z.infer<typeof UpdateCountriesSchema>,
    data: {
    ...
    if (mode === 'edit') {
        return Promise.all([
            await dbInstance
                .update(CountryTable)
                .set({ regionId: null, updatedAt: new Date(), updatedBy: session.user.id })
