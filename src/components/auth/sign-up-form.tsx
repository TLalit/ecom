...
export const SingUpForm = () => {
  ...
  const signUpMutation = useMutation({
    ...
    mutationFn: (data: PostSignUpRequest) =>
      axios.post<PostSignUpResponse>("/api/sign-up", data),
    onSuccess: (_, variables) =>
      signInWithCredentials({
        email: variables.email,
        password: variables.password,
      }),
    onError: errorHandler(({ message }) =>
      setSignUpErrorMessage(message ?? "Unknown error"),
    ),
  });
    ...
    <Form {...form}>
      <form onSubmit={onSubmit} className="flex flex-col gap-4">
        {signUpErrorMessage && (
          <Alert
            variant="destructive"
            className="flex items-center [&>svg]:top-2"
          >
