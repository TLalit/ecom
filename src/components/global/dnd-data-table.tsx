...
/**
 * this component is still work in progress as may not work as expected
 *
 */
export interface DnDDataTableProps<TData, TValue = any> {
  ...
  searchKey?: string;
  onDataChange: (items: TData[]) => Promise<void>;
}
...
export function DnDDataTable<TData extends DataTableState, TValue>({
  columns,
  data = [],
  loading = false,
  ...
  return (
    <ScrollArea className="relative flex-1 rounded">
      <Table>
        ...
        <TableBody>
          <Sortable value={data} onValueChange={onDataChange}>
            {loading ? (
              Array(1)
                .fill(null)
                .map((_, idx) => (
